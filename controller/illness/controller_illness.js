  ...
  try {
    ...
    if (resultIllness) {
      ...
      if (resultIllness.length > 0) {
        ...
        MESSAGES.DEFAULT_HEADER.illness = resultIllness
...
const listIllnessByType = async function (type, id_child) {
  let MESSAGES = JSON.parse(JSON.stringify(DEFAULT_MESSAGES))

  try {
    let resultIllness = await illnessDAO.getIllnessByType(type, id_child)
      ...
      if (!validar) {
        ...
        if (validarId.status_code == 200) {
          ...
          if (resultIllness) {
            ...
            MESSAGES.DEFAULT_HEADER.illness = illness
...
const validarDados = async function (illness) {
  ...
  else if (illness.illness_type !== "acute" && illness.illness_type !== "chronic" ) {
